...
Migrate incrementally—better-convex works alongside vanilla Convex. Convert functions one at a time while keeping existing code working.
...
```tsx title="src/lib/convex/server.ts"
