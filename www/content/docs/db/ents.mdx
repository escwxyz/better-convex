...
```ts
posts: defineEnt({})
  .edges('tags', { to: 'tags', table: 'postTags', field: 'postId' }),

tags: defineEnt({})
  .edges('posts', { to: 'posts', table: 'postTags', field: 'tagId' }),
```
...
When using [triggers](/docs/server/triggers), wrap the db before creating ents:
